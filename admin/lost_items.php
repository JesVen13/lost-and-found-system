...
if(isset($_POST['approval_action'])){
    ...
    if($action == 'approve'){
        $con->query("UPDATE lost_items SET approval_status='approved' WHERE lost_id=$lost_id");
    } elseif($action == 'reject'){
        $con->query("UPDATE lost_items SET approval_status='rejected' WHERE lost_id=$lost_id");
    }
...
if(isset($_GET['delete']) && isset($_GET['lost_id'])){
    $lost_id = intval($_GET['lost_id']);
    $con->query("DELETE FROM lost_items WHERE lost_id=$lost_id");
    header("Location: lost_items.php");
    exit;
}

// Fetch lost items with reporter info
$lost_items = $con->query("
...
// Get statistics
$total_items = $con->query("SELECT COUNT(*) as count FROM lost_items")->fetch_assoc()['count'];
$pending_approval = $con->query("SELECT COUNT(*) as count FROM lost_items WHERE approval_status='pending'")->fetch_assoc()['count'];
$found_items = $con->query("SELECT COUNT(*) as count FROM lost_items WHERE status='found'")->fetch_assoc()['count'];
$active_items = $con->query("SELECT COUNT(*) as count FROM lost_items WHERE status='lost' AND approval_status='approved'")->fetch_assoc()['count'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Lost Items Management - Lost & Found System</title>
...
<div class="main-container">
    ...
    <div class="page-header">
        <h1><i class="fas fa-box-open me-3"></i>Lost Items Management</h1>
        <p>View, manage, and approve all lost item reports</p>
    </div>
    ...
    <div class="stats-container">
        ...
        <div class="stat-card found">
            ...
            <div class="stat-number"><?= $found_items ?></div>
    ...
    <div class="content-card">
        ...
        <div class="card-body-custom">
            ...
            <div class="filter-tabs">
                ...
                <button class="filter-tab" onclick="filterItems('lost')">
                    <i class="fas fa-search me-1"></i> Lost
                </button>
                <button class="filter-tab" onclick="filterItems('found')">
            ...
            <div class="search-box">
                ...
                <input type="text" id="searchInput" placeholder="Search by item name, location, or reporter..." onkeyup="searchTable()">
            </div>
                            ...
                            <tr data-status="<?= $item['status'] ?>" data-approval="<?= $item['approval_status'] ?>">
                                ...
                                <td>
                                    <span class="badge-modern badge-<?= $item['status'] ?>">
                                        <i class="fas fa-<?= $item['status'] == 'found' ? 'check-circle' : 'search' ?> me-1"></i>
                                        <?= ucfirst($item['status']) ?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <a href="view_lost_item.php?lost_id=<?= $item['lost_id'] ?>" class="btn-action btn-view">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    
                                    <a href="?delete=1&lost_id=<?= $item['lost_id'] ?>" 
