...
$id = intval($_GET['id']);
$res = $con->query("SELECT * FROM lost_items WHERE lost_id=$id AND reporter_id=".$_SESSION['user_id']);
...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if(!$error && $item_name && $date_lost && $color && $location) {
        $stmt = $con->prepare("UPDATE lost_items SET item_name=?, date_lost=?, color=?, location_lost=?, description=?, contact_info=?, image_path=?, approval_status='pending' WHERE lost_id=?");
        $stmt->bind_param("sssssssi", $item_name, $date_lost, $color, $location, $description, $contact_info, $image_path, $id);
        
        if($stmt->execute()) {
            $success = "Item updated successfully! Waiting for staff approval.";
            // Refresh item data
            $res = $con->query("SELECT * FROM lost_items WHERE lost_id=$id");
            $item = $res->fetch_assoc();
        } else {
    ...
    <div class="form-card">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="date" name="date_lost" max="<?= date('Y-m-d') ?>" value="<?= $item['date_lost'] ?>" required>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="text" name="location" placeholder="e.g., Building A - Room 101" value="<?= htmlspecialchars($item['location_lost']) ?>" required>
            </div>
